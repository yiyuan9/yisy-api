...
const ApiDashboard = async () => {
  ...
  const serializedApiRequests = userRequests.map((req) => ({
    ...req,
  ...
  return <div className="container flex flex-col gap-6">
    ...
    <div className="flex flex-col md:flex-row gap-4 justify-center md:justify-start items-center">
      ...
      <ApiKeyOptions apiKeyId={activeApiKey.id} apiKeyString={activeApiKey.key} />
    </div>
    <Paragraph className='text-center md:text-left mt-4 -mb-4'>Your API history: </Paragraph>

    <Table userRequests={serializedApiRequests} />
  </div>
}
